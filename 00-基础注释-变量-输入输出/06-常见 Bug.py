...
默认参数只在函数定义时初始化一次，因此被多次调用时，共享同一个对象。
	•	影响范围： 不仅是列表和字典，任何可变对象（如自定义类实例）都可能出现类似问题。
"""
...
额外补充：
	•	小整数对象池和字符串缓存导致了某些值的 is 比较结果可能为 True。
	•	避免使用 is 比较可变数据类型的值（如列表、字典等）。
"""
